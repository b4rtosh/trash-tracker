...
def request_distance(latitude1, longitude1, latitude2, longitude2):
    ...
    try:
        ...
        if (data and isinstance(data.get("routes"), list) and len(data["routes"]) > 0 and 
            ...
            logging.warning(f"JSON response from {url} did not have the expected structure or 'distance' key. Response: {data}")
            return None # Indicate that distance could not be found
            
    except requests.exceptions.HTTPError as http_err:
        logging.error(f"HTTP error occurred for {url}: {http_err} - Response: {api_response.text if 'api_response' in locals() else 'N/A'}")
        return None 
        ...
        for j in range(len(route_points)):
            if i == j:
                ...
                try:
                    ...
                    if distance is None:
                        logging.warning(f"Could not retrieve distance between point {route_points[i].id} ({route_points[i].latitude}, {route_points[i].longitude}) and point {route_points[j].id} ({route_points[j].latitude}, {route_points[j].longitude})")
                except Exception as e:
                    logging.error(f"Unexpected error calling request_distance for points {route_points[i].id} and {route_points[j].id}: {e}")
...
def optimize_points(route_id):
    ...
    print(order)
    for index, id in enumerate(order, start=0):
        route_points[id].sequence_number = index
    return dist
